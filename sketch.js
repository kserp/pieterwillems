...
let soundSketch = (p) => {
  ...
  p.draw = function () {
    ...
    if(p.millis()-lastClick>clickInterval){
    ...
    if(p.millis()-lastBurstTime>burstInterval){
      if(p.random()<0.5){ burstEnv.play(noise);
        burstReverb.process(noise,0.1,2); burstReverb.drywet(0.2); burstReverb.connect(masterBus);
      }
      burstInterval=p.random(800,1600); lastBurstTime=p.millis();
    }

    if(doubleSpeed&&p.millis()>doubleSpeedEnd){ doubleSpeed=false; }

    if(p.millis()-lastHighNote>1700){
      let baseFreq=698.46; if(p.random()<0.15) baseFreq=659.25; sineHigh.freq(baseFreq);
      if(p.random()<0.5){ highEnv.setADSR(0.01,0.05,0,0.1); highEnv.setRange(0.2,0); }
      else { highEnv.setADSR(0.02,0.2,0,0.4); highEnv.setRange(0.3,0); }
      highEnv.play(sineHigh); lastHighNote=p.millis();
    }

    if(inRun&&p.millis()-lastRunClick>50){
      hiClickFilter.freq(p.random(8000,10000)); hiClickEnv.play(hiClickOsc);
      runStep++; lastRunClick=p.millis(); if(runStep>=16) inRun=false;
    }
  };

  function triggerLowKick(vol){
    lowEnv.setADSR(0.01,0.01,0.7,0.2); lowEnv.setRange(3,0); lowEnv.play(sineLow);
    lowDistortion.set(p.random(0.0,0.003),'2x');
    kickClickFilter.freq(p.random(2500,5000)); kickClickEnv.play(kickClickNoise);
  }
};